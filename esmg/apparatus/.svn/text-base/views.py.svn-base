...
class MovePageManipulator(forms.Manipulator):
    def __init__(self, movingpage):
        choices = []
        movingpageobject = Page.objects.get(id=movingpage)
        p = Page.objects.get(tree=movingpageobject.tree, is_tree_top=True)
        choices.append((p.id, p.uri))
        indent = '- '
...
def treelist(indent,page,movingpage,choices):
    ...
    pages = p.child_set.exclude(id=movingpage).filter(disabled=False)
    for child in pages:
...
def editpage(request, **kwargs):
    ...
    if ppage:
        ...
        # fix for case of super editor editing someone else's personal page
        person_editor = not create and not mine and p.is_tree_top
    else:
        personcreate = personhome = person_editor = False
    ...
    if request.POST:
        ...
        if homecreate or home:
            ...
            if not create and p.is_tree_top and not p.is_person_page:
                ...
                if not p.is_tree_top and identifier == '':
                    errors.setdefault('identifier', ['You \
        ...
        if not identifier=='' and not slug_re.search(identifier):
                errors.setdefault('identifier',['identifiers must consist of\
                ...
                if create:
                    ...
                    if homecreate:
                        tree=Tree(type='top', identifier='', group=None, \
                                person=None )
                        tree.save()
                    if not Page.objects.filter(parent__id=pid,identifier=identifier).count()==0:
                       return error(request, "A page with identifier " +
                               identifier + " already exists here. Did you use \
                               the back button to go back to editing a page \
                               after creating it? That unavoidably takes you \
                               back to the 'create' page, rather than to the \
                               edit page. Try to avoid using the back button \
                               when using forms.")
                    newpage = Page(
                    ...
                    if p:
                        newpage.is_tree_top=False
                    if p.is_person_page:
                        newpage.is_person_page=True
                    else:
                        newpage.is_person_page=False
                    newpage.save()
                else:
...
def movepage(request, **kwargs):
    ...
    if not kwargs["uri"] == '/' and not personpage:
        if not personsubpage:
            uri = "/" + kwargs["uri"] + "/"
        else:
            uri = "/people/" + kwargs["uri"] + "/"
    else:
